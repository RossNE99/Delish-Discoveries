function openRecipeModal(e, recipe) {
    ...
    if(!recipe){ //If no recipe was passed into this function then it was called from the search page, below is how we find the correct recipe that the user clicked on
        var recipeId = $(e.currentTarget).data("recipeid")  //Get the uniqe id for the recipe that was clicked
        recipe = loadedRecipes.find(recipe => recipe._links.self.href === recipeId);    //retrive that recipe from the loadedrecipe array, using recipeID to identify the correct one 
    } else {
    ...
    $.each(ingredientLines, function (indexInArray, ingredient) { //create ingredint list
        var ingredientLI = $("<li>", {
    ...
    $("#viewCookingInstructionsBtn").on('click', function(){ window.open(url, '_blank')});  //open recipe instructions in new tab when view recipe button is clicked

    renderGraph(recipe) //create the chart
    $('#recipeModal').modal('show');    //add the chart to the page 

    $('#planMealDateInput').daterangepicker({   //event listerner for date picker
        drops: "up",
        autoUpdateInput: true,
        singleDatePicker: true,         //date picker options
        showDropdowns: true,
        autoApply: true,
      }, function(mealDay){ savePlannedRecipeToLocalStorage(mealDay)}   //callback function is called one user selects a day to plan meal for
    )

    $("#planMealForToday").on("click", function() {savePlannedRecipeToLocalStorage(dayjs())}) //if planForTodaybtn is pressed then save the meal for the current day

    function savePlannedRecipeToLocalStorage(mealDay) { //function to handel saving the recipe
        var PlannedMealObj = {mealDay: mealDay.format("YYYY-MM-DD"), recipe}    //create the plannedMeal object
        var prevPlannedMeals = JSON.parse(localStorage.getItem("plannedMeals")) || []   //get local stoarage, if there is none then retuen an empty array
    
        localStorage.setItem("plannedMeals", JSON.stringify([...prevPlannedMeals, PlannedMealObj])) //save the new meal object into local storage
        showToast("Meal added to calendar!", `${recipe.recipe.label}, has been planned for ${dayjs(mealDay).format("D MMM")}`, `fa-save`, 'blue') //show a toast to the user to let them know that there meal has been saved
        }
...
function addToFavorites(recipe) {
    ...
    showToast("Meal Saved!", `${recipe.recipe.label}, has been saved to favs`, `fa-save`, 'yellow') //show a toast to the user to let them know that there meal has been saved
