var calendar;
function rednderCal() {
  //cunction to render the callender to the page
  var calendarEl = document.getElementById("calendar");

  calendar = new FullCalendar.Calendar(calendarEl, {
    eventClick: function (info) {
      //function to run when a event on the callender is clicked
    ...
    events: createCalData(
      JSON.parse(localStorage.getItem("plannedMeals")) || []
    ), //get the data from local stotage and pass it into callander
  });
  calendar.render();
}

function createCalData(ls) {
  //function to convert local storage data into a format/shape that can be read by the callendar
